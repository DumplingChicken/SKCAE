...
def ML_train(inp, numberx, numbery, path, offset = 1, activ = 'relu',
    ...
    inp = np.reshape(inp, (-1, numberx, numbery, 1),order='F')
    x_train, x_test = train_test_split(inp, test_size=0.205, shuffle=False, random_state=1)
    ...
    o = (Conv2D(1, (fs,fs), padding='same', name='Output'))(conv12)
...
def ML_pred(ori, numberx, numbery, model):
    ...
    ori = np.reshape(ori, (-1, numberx, numbery, 1),order='F')
...
def model_rec(ori, numberx, numbery, model):
    ori = np.transpose(ori)
    ori = np.reshape(ori, (-1, numberx, numbery, 1),order='F')
    pred = model.predict(ori)[0]
    pred = np.reshape(pred, (-1, numberx*numbery),order='F')
    pred = np.transpose(pred)
    return pred

def model_pred(ori, numberx, numbery, model):
    ori = np.transpose(ori)
    ori = np.reshape(ori, (-1, numberx, numbery, 1),order='F')
    pred = model.predict(ori)[1]
    pred = np.transpose(pred)
    return pred
